...
class RAGAgent(LLMAgent):
    def __init__(self, *args, **kwargs):
        # Always read OPENAI_API_KEY from the OS directly for Weaviate, ignoring whatever LLMAgent does.
        weaviate_openai_key = os.environ.get("OPENAI_API_KEY", "")
        ...
        self.client = weaviate.connect_to_weaviate_cloud(
            ...
            headers={"X-OpenAI-Api-Key": weaviate_openai_key},
        )
    ...
    def build_messages(self, observation):
        ...
        if self.rag_top_k > 0:
            response = self.collection.query.near_text(
                query=observation, limit=self.rag_top_k
            )
